...
function Bar({ pct, color }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 h-2 bg-sunken rounded-full">
    ...
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-ink-4 text-sm mt-1">{cert.fullName} · {cert.code}</p>
        </div>
        <div className={`text-xs px-3 py-1.5 rounded-full shrink-0 whitespace-nowrap ${overallAvg !== null && overallAvg >= passingPct ? 'bg-emerald-600/10 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-600/30 dark:border-emerald-700/40' : 'bg-sunken text-ink-4 border border-line-2'}`}>
          Passing: {passingPct}%
        </div>
      </div>
            ...
            return (
              <div key={d.id} className="card">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <span className="text-xs text-ink-5 mr-2">D{d.id} · {d.percent}%</span>
                ...
                {best !== null ? (
                  <>
                    ...
                    <div className="flex gap-4 mt-2 text-xs text-ink-5">
                      <span>Best: <span className="text-ink-3">{best}%</span></span>
                      <span>Avg: <span className="text-ink-3">{avg}%</span></span>
                      <span>{attempts.length} attempt{attempts.length !== 1 ? 's' : ''}</span>
                    </div>
                  </>
                ) : (
                  <p className="text-ink-5 text-sm">No attempts yet</p>
                )}
              </div>
            )
          })}
          ...
          {data.domains.map((d) => {
            ...
            return (
              <div key={d.id} className="flex items-center gap-4">
                <div className="w-28 text-xs text-ink-5 shrink-0">D{d.id} · {d.name.split(' ')[0]}</div>
                <div className="flex-1"><Bar pct={pct} color={cert.color} /></div>
                <div className="text-xs text-ink-5 w-16 text-right shrink-0">{mastered}/{cards.length}</div>
              </div>
            )
          })}
        </div>

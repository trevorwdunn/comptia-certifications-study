  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1 text-sm">Test your knowledge by domain or simulate the full exam</p>
      </div>

      <Link to={`/${certId}/quiz/all`} className="card border border-slate-600 hover:border-slate-400 transition-colors flex items-center justify-between">
        <div>
          <div className="font-semibold">Mixed Exam — All Domains</div>
          <div className="text-sm text-slate-400 mt-0.5">{data.questions.length} questions · Simulates the real exam</div>
        </div>
                ...
                <Link key={d.id} to={`/${certId}/quiz/${d.id}`}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500">{qCount} questions · {d.percent}% of exam</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    {qData?.bestScore != null && (
                      <span className={`text-xs font-semibold ${qData.bestScore >= 80 ? 'text-emerald-400' : qData.bestScore >= 60 ? 'text-amber-400' : 'text-red-400'}`}>
